/**
 * AntiAlcohol View - Rendu HTML pour l'addiction à l'alcool
 */

import { AddictionBaseView } from '../../AddictionBase/view/addiction-base-view.js';
import { SLOPE_SIGNALS, SLOPE_STEPS, TRIGGERS, ENVIRONMENT_RULES, UI_LABELS } from '../data/antialcohol-data.js';

export class AntiAlcoholView extends AddictionBaseView {
    constructor() {
        super('alcohol');
    }
        ...
        const stepsHtml = Object.entries(SLOPE_STEPS).map(([key, step], idx) => {
            ...
            return `
                <div class="slope-step ${stepStatus}" data-step="${key}" data-idx="${idx}">
                    ...
                    <button class="btn btn-primary btn-block step-btn" onclick="AntiAlcohol.completeStep('${key}')">Fait</button>
                </div>
            `;
        }).join('');

        const signalsHtml = Object.entries(SLOPE_SIGNALS).map(([key, signal]) => `
            <button class="chip small" onclick="AntiAlcohol.logWithSignal('${key}')">${signal[lang] || signal.fr}</button>
        `).join('');
        ...
        this.slopeModalEl.innerHTML = `
            <div class="modal-content slope-modal slope-advanced">
                ...
                <div class="slope-completed" style="display: none;">
                    ...
                    <button class="btn btn-primary btn-large" onclick="AntiAlcohol.confirmSlope()">${l.confirmButton}</button>
                </div>
            </div>
        `;
        
        this.slopeModalEl.classList.add('active');
        this.setupOverlayClose(this.slopeModalEl, () => window.AntiAlcohol.closeSlopeModal());
    }
    ...
    renderConfigModal(lang, customTriggers, activeRules) {
        ...
        const triggersHtml = Object.entries(TRIGGERS).map(([key, labels]) => `
            <button class="chip ${customTriggers.includes(key) ? 'active' : ''}" onclick="AntiAlcohol.toggleTrigger('${key}')">${labels[lang] || labels.fr}</button>
        `).join('');
        ...
        this.configModalEl.innerHTML = `
            <div class="modal-content config-modal">
                <button class="modal-close" onclick="AntiAlcohol.closeConfigModal()">×</button>
                <h2>🍷 ${l.configTitle}</h2>
                ...
                <button class="btn btn-primary btn-block" onclick="AntiAlcohol.saveConfig()">✓ Enregistrer</button>
            </div>
        `;
        
        this.configModalEl.classList.add('active');
        this.setupOverlayClose(this.configModalEl, () => window.AntiAlcohol.closeConfigModal());
    }
}
