/**
 * AntiFood View - Rendu HTML pour l'addiction alimentaire compulsive
 */

import { AddictionBaseView } from '../../AddictionBase/view/addiction-base-view.js';
import { SLOPE_SIGNALS, SLOPE_STEPS, TRIGGERS, ENVIRONMENT_RULES, UI_LABELS } from '../data/antifood-data.js';

export class AntiFoodView extends AddictionBaseView {
    constructor() { super('food'); }

    renderSlopeContent(lang, stoppedCount, tips, state = null, selectedAddictionId = 'food') {
        ...
        const selectorHtml = state && state.addictions && state.addictions.length > 1 
            ? this.renderAddictionSelector(state, selectedAddictionId, 'AntiFood.onAddictionChange')
            : '';
        ...
        const stepsHtml = Object.entries(SLOPE_STEPS).map(([key, step], idx) => {
            ...
            return `<div class="slope-step ${stepStatus}" data-step="${key}" data-idx="${idx}">
                ...
                <button class="btn btn-primary btn-block step-btn" onclick="AntiFood.completeStep('${key}')">Fait</button>
            </div>`;
        }).join('');

        const signalsHtml = Object.entries(SLOPE_SIGNALS).map(([key, signal]) => 
            `<button class="chip small" onclick="AntiFood.logWithSignal('${key}')">${signal[lang] || signal.fr}</button>`
        ).join('');
        ...
        this.slopeModalEl.innerHTML = `<div class="modal-content slope-modal slope-advanced">
            <button class="modal-close" onclick="AntiFood.closeSlopeModal()">×</button>
            ${selectorHtml}
            ...
            <div class="slope-completed" style="display: none;">
                <div class="completed-icon">🎉</div><h3>Bravo !</h3><p>Tu as écouté ton corps, pas tes émotions !</p>
                <button class="btn btn-primary btn-large" onclick="AntiFood.confirmSlope()">${l.confirmButton}</button>
            </div>
        </div>`;
        
        this.slopeModalEl.classList.add('active');
        this.setupOverlayClose(this.slopeModalEl, () => window.AntiFood.closeSlopeModal());
    }
    ...
    renderConfigModal(lang, customTriggers, activeRules) {
        ...
        const triggersHtml = Object.entries(TRIGGERS).map(([key, labels]) => 
            `<button class="chip ${customTriggers.includes(key) ? 'active' : ''}" onclick="AntiFood.toggleTrigger('${key}')">${labels[lang] || labels.fr}</button>`
        ).join('');
        ...
        this.configModalEl.innerHTML = `<div class="modal-content config-modal">
            <button class="modal-close" onclick="AntiFood.closeConfigModal()">×</button>
            <h2>🍪 ${l.configTitle}</h2>
            ...
            <button class="btn btn-primary btn-block" onclick="AntiFood.saveConfig()">✓ Enregistrer</button>
        </div>`;
        
        this.configModalEl.classList.add('active');
        this.setupOverlayClose(this.configModalEl, () => window.AntiFood.closeConfigModal());
    }
}
