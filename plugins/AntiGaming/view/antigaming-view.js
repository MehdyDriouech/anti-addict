/**
 * AntiGaming View - Rendu HTML pour l'addiction aux jeux vidéo
 */

import { AddictionBaseView } from '../../AddictionBase/view/addiction-base-view.js';
import { SLOPE_SIGNALS, SLOPE_STEPS, TRIGGERS, ENVIRONMENT_RULES, UI_LABELS } from '../data/antigaming-data.js';

export class AntiGamingView extends AddictionBaseView {
    constructor() { super('gaming'); }

    renderSlopeContent(lang, stoppedCount, tips, state = null, selectedAddictionId = 'gaming') {
        ...
        const selectorHtml = state && state.addictions && state.addictions.length > 1 
            ? this.renderAddictionSelector(state, selectedAddictionId, 'AntiGaming.onAddictionChange')
            : '';
        ...
        const stepsHtml = Object.entries(SLOPE_STEPS).map(([key, step], idx) => {
            ...
            return `<div class="slope-step ${stepStatus}" data-step="${key}" data-idx="${idx}">
                ...
                <button class="btn btn-primary btn-block step-btn" onclick="AntiGaming.completeStep('${key}')">Fait</button>
            </div>`;
        }).join('');

        const signalsHtml = Object.entries(SLOPE_SIGNALS).map(([key, signal]) => 
            `<button class="chip small" onclick="AntiGaming.logWithSignal('${key}')">${signal[lang] || signal.fr}</button>`
        ).join('');
        ...
        this.slopeModalEl.innerHTML = `<div class="modal-content slope-modal slope-advanced">
            <button class="modal-close" onclick="AntiGaming.closeSlopeModal()">×</button>
            ${selectorHtml}
            ...
            <div class="slope-completed" style="display: none;">
                <div class="completed-icon">🎉</div><h3>Bravo !</h3><p>Tu as repris le contrôle. Le monde réel t'attend !</p>
                <button class="btn btn-primary btn-large" onclick="AntiGaming.confirmSlope()">${l.confirmButton}</button>
            </div>
        </div>`;
        
        this.slopeModalEl.classList.add('active');
        this.setupOverlayClose(this.slopeModalEl, () => window.AntiGaming.closeSlopeModal());
    }
    ...
    renderConfigModal(lang, customTriggers, activeRules) {
        ...
        const triggersHtml = Object.entries(TRIGGERS).map(([key, labels]) => 
            `<button class="chip ${customTriggers.includes(key) ? 'active' : ''}" onclick="AntiGaming.toggleTrigger('${key}')">${labels[lang] || labels.fr}</button>`
        ).join('');
        ...
        this.configModalEl.innerHTML = `<div class="modal-content config-modal">
            <button class="modal-close" onclick="AntiGaming.closeConfigModal()">×</button>
            <h2>🎮 ${l.configTitle}</h2>
            ...
            <button class="btn btn-primary btn-block" onclick="AntiGaming.saveConfig()">✓ Enregistrer</button>
        </div>`;
        
        this.configModalEl.classList.add('active');
        this.setupOverlayClose(this.configModalEl, () => window.AntiGaming.closeConfigModal());
    }
}
