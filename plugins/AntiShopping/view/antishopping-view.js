/**
 * AntiShopping View - Rendu HTML pour l'addiction aux achats compulsifs
 */

import { AddictionBaseView } from '../../AddictionBase/view/addiction-base-view.js';
import { SLOPE_SIGNALS, SLOPE_STEPS, TRIGGERS, ENVIRONMENT_RULES, UI_LABELS } from '../data/antishopping-data.js';
...
export class AntiShoppingView extends AddictionBaseView {
    ...
    renderSlopeContent(lang, stoppedCount, tips) {
        ...
        const stepsHtml = Object.entries(SLOPE_STEPS).map(([key, step], idx) => {
            ...
            return `<div class="slope-step ${stepStatus}" data-step="${key}" data-idx="${idx}">
                ...
                <button class="btn btn-primary btn-block step-btn" onclick="AntiShopping.completeStep('${key}')">Fait</button>
            </div>`;
        }).join('');

        const signalsHtml = Object.entries(SLOPE_SIGNALS).map(([key, signal]) => 
            `<button class="chip small" onclick="AntiShopping.logWithSignal('${key}')">${signal[lang] || signal.fr}</button>`
        ).join('');
        ...
        this.slopeModalEl.innerHTML = `<div class="modal-content slope-modal slope-advanced">
            ...
            <div class="slope-completed" style="display: none;">
                <div class="completed-icon">🎉</div><h3>Bravo !</h3><p>Tu as gardé le contrôle. Ton compte te remercie !</p>
                <button class="btn btn-primary btn-large" onclick="AntiShopping.confirmSlope()">${l.confirmButton}</button>
            </div>
        </div>`;
        
        this.slopeModalEl.classList.add('active');
        this.setupOverlayClose(this.slopeModalEl, () => window.AntiShopping.closeSlopeModal());
    }
    ...
    renderConfigModal(lang, customTriggers, activeRules) {
        ...
        const triggersHtml = Object.entries(TRIGGERS).map(([key, labels]) => 
            `<button class="chip ${customTriggers.includes(key) ? 'active' : ''}" onclick="AntiShopping.toggleTrigger('${key}')">${labels[lang] || labels.fr}</button>`
        ).join('');
        ...
        this.configModalEl.innerHTML = `<div class="modal-content config-modal">
            <button class="modal-close" onclick="AntiShopping.closeConfigModal()">×</button>
            <h2>🛒 ${l.configTitle}</h2>
            ...
            <button class="btn btn-primary btn-block" onclick="AntiShopping.saveConfig()">✓ Enregistrer</button>
        </div>`;
        
        this.configModalEl.classList.add('active');
        this.setupOverlayClose(this.configModalEl, () => window.AntiShopping.closeConfigModal());
    }
}
