/**
 * AntiGambling View - Rendu HTML pour l'addiction au jeu d'argent
 */

import { AddictionBaseView } from '../../AddictionBase/view/addiction-base-view.js';
import { SLOPE_SIGNALS, SLOPE_STEPS, TRIGGERS, ENVIRONMENT_RULES, UI_LABELS } from '../data/antigambling-data.js';

export class AntiGamblingView extends AddictionBaseView {
    constructor() { 
        super('gambling'); 
    }

    renderSlopeContent(lang, stoppedCount, tips, state = null, selectedAddictionId = 'gambling') {
        ...
        const selectorHtml = state && state.addictions && state.addictions.length > 1 
            ? this.renderAddictionSelector(state, selectedAddictionId, 'AntiGambling.onAddictionChange')
            : '';
        ...
        const stepsHtml = Object.entries(SLOPE_STEPS).map(([key, step], idx) => {
            ...
            return `<div class="slope-step ${stepStatus}" data-step="${key}" data-idx="${idx}">
                ...
                <button class="btn btn-primary btn-block step-btn" onclick="AntiGambling.completeStep('${key}')">${l.completeStep}</button>
            </div>`;
        }).join('');

        const signalsHtml = Object.entries(SLOPE_SIGNALS).map(([key, signal]) => 
            `<button class="chip small" onclick="AntiGambling.logWithSignal('${key}')">${signal[lang] || signal.fr}</button>`
        ).join('');
        ...
        this.slopeModalEl.innerHTML = `<div class="modal-content slope-modal slope-advanced">
            <button class="modal-close" onclick="AntiGambling.closeSlopeModal()">×</button>
            ${selectorHtml}
            ...
            <div class="slope-completed" style="display: none;">
                <div class="completed-icon">🎉</div><h3>${l.completed}</h3><p>${l.completedMessage}</p>
                <button class="btn btn-primary btn-large" onclick="AntiGambling.confirmSlope()">${l.confirmButton}</button>
            </div>
        </div>`;
        
        this.slopeModalEl.classList.add('active');
        this.setupOverlayClose(this.slopeModalEl, () => window.AntiGambling.closeSlopeModal());
    }
    ...
    renderConfigModal(lang, customTriggers, activeRules) {
        ...
        const triggersHtml = Object.entries(TRIGGERS).map(([key, labels]) => 
            `<button class="chip ${customTriggers.includes(key) ? 'active' : ''}" onclick="AntiGambling.toggleTrigger('${key}')">${labels[lang] || labels.fr}</button>`
        ).join('');
        ...
        this.configModalEl.innerHTML = `<div class="modal-content config-modal">
            <button class="modal-close" onclick="AntiGambling.closeConfigModal()">×</button>
            <h2>🎰 ${l.configTitle}</h2>
            <div class="config-section"><h4>Déclencheurs</h4><div class="trigger-chips">${triggersHtml}</div></div>
            ${rulesHtml ? `<div class="config-section"><h4>Règles d'environnement</h4><div class="checklist">${rulesHtml}</div></div>` : ''}
            <button class="btn btn-primary btn-block" onclick="AntiGambling.saveConfig()">✓ Enregistrer</button>
        </div>`;
        
        this.configModalEl.classList.add('active');
        this.setupOverlayClose(this.configModalEl, () => window.AntiGambling.closeConfigModal());
    }
}
