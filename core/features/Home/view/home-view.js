...
export class HomeView {
    ...
    /**
     * Rend le widget de coaching (Coaching V2 - adaptatif selon mode)
     * @param {Object} insights - Insights hebdomadaires (pour compatibilité, déprécié)
    ...
    renderCoachingWidget(insights, lang) {
        ...
        // Utiliser le nouveau système adaptatif de Coaching V2
        if (typeof Coaching !== 'undefined' && Coaching.renderWidget) {
            return Coaching.renderWidget(state);
        }
        
        // Fallback vers ancien système si Coaching non disponible
        const mode = state?.coaching?.mode || 'stability';
        if (mode === 'silent') {
            return ''; // Mode silent: ne pas afficher
        }
        
        // Pour observer: très discret
        if (mode === 'observer') {
            // Récupérer summary
            let summary = null;
            ...
            // Afficher icône seule pour observer
            return `<div class="coaching-widget observer-mode" onclick="if (typeof Coaching !== 'undefined' && Coaching.openCoaching) { Coaching.openCoaching(window.state); }" title="${I18n.t('coaching') || 'Coaching'}">🧠</div>`;
        }
